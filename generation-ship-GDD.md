...
- **Blank-cheque nudges** come from control terminals and arrive *unspecified*. The world produces the capacity; the player supplies the intent. Filling them in is most of what submitting a turn actually is — *this one is farming +1, this one is logging +1* — which is how player agency enters a system that otherwise runs itself. Blank nudges are **spent where they are produced**: authority is bureaucratic and does not travel. A blank nudge belongs to **the player whose terminal minted it**, and that player is who fills it in — the terminal carries the ownership, so two players' terminals in one settlement stay each other's business. (Single-player makes this moot today, but it is the standing rule; see §8 and the implementation guide.)
...
Two axes are worth keeping apart. *How* nudges combine is player-agnostic — the priority score just sums every delta, so there is no arbitration and no owner on a place. *Who* may spend a given blank-cheque nudge is a separate, ownership question, and it follows the **control terminal** that minted it: your terminal's blank nudges are yours to fill in, a rival's are theirs, even side by side in one location. Superposition removes the fight over who a place belongs to; terminal ownership is only about which hand fills in which blank cheque.
